...
export type OrganizationMembership = { organizationId: string; organizationName: string; organizationType: string; membershipType: string; jobTitle: string | null; status: string };
export type SiteAccess = { siteId: string; code: string; name: string; accessLevel: 'view' | 'collaborate' };
export type AccessContext = { userId: string; persona: Persona | null; accountStatus: AccountStatus; aal: AuthenticatorAssuranceLevel; organizations: OrganizationMembership[]; sites: SiteAccess[]; permissions: Permission[] };
