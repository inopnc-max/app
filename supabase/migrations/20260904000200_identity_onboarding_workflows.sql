...
create or replace function public.create_internal_invitation(p_token_hash text,p_target_persona public.persona,p_target_hint text,p_expires_at timestamptz) returns uuid language plpgsql security definer set search_path='' as $$ declare i uuid; begin if not private.is_admin_aal2_self() or p_target_persona not in ('worker','site_manager','production_manager') or length(p_token_hash)<64 or p_expires_at<=now() then raise exception 'invalid invitation'; end if; insert into public.invitations(token_hash,target_persona,target_hint,expires_at,created_by) values(p_token_hash,p_target_persona,p_target_hint,p_expires_at,auth.uid()) returning id into i; insert into public.audit_events(actor_id,event_type,metadata) values(auth.uid(),'invitation.created',jsonb_build_object('invitation_id',i,'target_persona',p_target_persona)); return i; end $$;
create or replace function public.revoke_internal_invitation(p_invitation_id uuid) returns void language plpgsql security definer set search_path='' as $$ begin if not private.is_admin_aal2_self() then raise exception 'forbidden'; end if; update public.invitations set revoked_at=now() where id=p_invitation_id and accepted_at is null and revoked_at is null and expires_at>now(); if not found then raise exception 'invalid invitation'; end if; insert into public.audit_events(actor_id,event_type,metadata) values(auth.uid(),'invitation.revoked',jsonb_build_object('invitation_id',p_invitation_id)); end $$;
create or replace function public.submit_partner_access_request(p_name text,p_company text,p_job_title text,p_contact text) returns uuid language plpgsql security definer set search_path='' as $$ declare i uuid; begin if not exists(select 1 from public.profiles where id=auth.uid() and persona is null and account_status='pending') then raise exception 'ineligible'; end if; insert into public.access_requests(profile_id,name,company,job_title,contact,status) values(auth.uid(),trim(p_name),trim(p_company),trim(p_job_title),nullif(trim(p_contact),''),'pending') returning id into i; insert into public.audit_events(actor_id,event_type,metadata) values(auth.uid(),'access_request.submitted',jsonb_build_object('request_id',i)); return i; end $$;
...
revoke all on function public.create_internal_invitation(text,public.persona,text,timestamptz),public.revoke_internal_invitation(uuid),public.submit_partner_access_request(text,text,text,text) from public,anon;
grant execute on function public.create_internal_invitation(text,public.persona,text,timestamptz),public.revoke_internal_invitation(uuid),public.submit_partner_access_request(text,text,text,text) to authenticated;
